	...
	<div id="wrapper">
		<?php
			...
			if ($_SERVER["REQUEST_METHOD"] == "POST") {
				...
				// 	 	</script>";
				if (!empty($_POST)) {
					if(mysqli_query($con, "UPDATE siswa SET first_name = '$first_name', last_name = '$last_name', tgl_lahir = '$tgl_lahir', alamat = '$alamat', no_hp = '$no_hp', wali_murid = '$wali_murid', hp_wali = '$hp_wali', kd_kelas = '$kd_kelas' WHERE nis = '$_POST[nis]' "))
					echo "<script>
						...
						echo "<script>
						  ...
						  echo "UPDATE siswa SET first_name = '$first_name', last_name = '$last_name', tgl_lahir = '$tgl_lahir', alamat = '$alamat', no_hp = '$no_hp', wali_murid = '$wali_murid', hp_wali = '$hp_wali', kd_kelas = '$kd_kelas' WHERE nis = '$_POST[nis]' ";
				}
			}
		?>
											...
											<div class="col-md-6">
		 									...
		 									<div class="col-md-6">
												...
												<option value="">-- Pilih Kelas --</option>
													<?php
														$qkelas = mysqli_query($con, "SELECT kelas.*, jurusan.jurusan FROM kelas LEFT JOIN jurusan ON kelas.id_jurusan=jurusan.id_jurusan");
														while ($x = mysqli_fetch_array($qkelas)) {
															// print_r($x);
														echo "<option value = '$x[0]' ".(isset($_POST['kd_kelas']) && $_POST['kd_kelas'] == $x[0] ? 'selected' : '')."> 
															$x[1] $x[4] $x[2]
															 </option>";
																}
															 ?>
